...
// MÉTODOS PÚBLICOS
FUZZYLIB_DLL bool Fuzzy::addFuzzyInput(FuzzyInput* fuzzyInput){
    fuzzyInputArray* aux;
...
FUZZYLIB_DLL bool Fuzzy::addFuzzyOutput(FuzzyOutput* fuzzyOutput){
...
FUZZYLIB_DLL bool Fuzzy::addFuzzyRule(FuzzyRule* fuzzyRule){
    fuzzyRuleArray* aux;
    // Alocando espaço na memória
...
FUZZYLIB_DLL bool Fuzzy::setInput(int fuzzyInputIndex, float crispValue){
    fuzzyInputArray *aux;
...
FUZZYLIB_DLL bool Fuzzy::fuzzify(){
...
FUZZYLIB_DLL bool Fuzzy::isFiredRule(int fuzzyRuleIndex){
    fuzzyRuleArray *aux;
    aux = this->fuzzyRules;
    while(aux != NULL){
...
FUZZYLIB_DLL float Fuzzy::defuzzify(int fuzzyOutputIndex){
    fuzzyOutputArray *aux;
...
// MÉTODOS PRIVADOS
FUZZYLIB_DLL void Fuzzy::cleanFuzzyInputs(fuzzyInputArray* aux){
    if(aux != NULL){
...
FUZZYLIB_DLL void Fuzzy::cleanFuzzyOutputs(fuzzyOutputArray* aux){
    if(aux != NULL){
        // Esvaziando a memória alocada
...
FUZZYLIB_DLL void Fuzzy::cleanFuzzyRules(fuzzyRuleArray* aux){
    if(aux != NULL){
        // Esvaziando a memória alocada
